<template>
	...
	>
		...
		<template v-slot:[`item.name`]="{item}">
			<router-link :to="`/balancer/${item.raw.id}`">{{item.raw.name}}</router-link>
		</template>
		...
		<template v-slot:[`item.ip`]="{item}">
			<v-list density="compact" variant="text" bg-color="transparent">
				<v-list-item variant="text" color="none" density="compact" v-for="ip in item.raw.ip" :title="ip" :key="ip"></v-list-item>
			</v-list>
		</template>
		...
		return{
			table: {
				...
				headers: [
					...
					{ title: 'Operating Status', align: 'start', key: 'state' },
					{ title: 'IP Address', align: 'start', key: 'ip' },
					{ title: 'Operating Status', align: 'start', key: 'status' },
					{ title: 'Gateways', align: 'start', key: 'gateways' },
					{ title: '', key: 'actions', align: 'end', sortable: false }
				],
				data: [
					{id: 1, check: false, name: 'netengi-loadbalancer1', state: 1, ip: ['95.163.248.0/22', '79.137.174.0/23'], status: 0, gateways: 'Yes'},
					{id: 2, check: false, name: 'netengi-loadbalancer2', state: 2, ip: ['95.163.248.0/22', '79.137.174.0/23'], status: 0, gateways: 'No'},
					{id: 3, check: false, name: 'netengi-loadbalancer3', state: 1, ip: ['95.163.248.0/22', '79.137.174.0/23'], status: 0, gateways: 'Yes'},
				]
			},
			modalItem: {},
			modalDelete: false,
